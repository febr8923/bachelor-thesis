#!/bin/bash
#SBATCH --job-name=um-benchmarking
#SBATCH --nodes=1
...
### CUDA
export CUDACXX=/user-environment/env/default/bin/nvcc

###
#export LD_LIBRARY_PATH=/users/fbrunne/miniconda3/envs/rmm_dev/lib:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=~/projects/deps/python311/lib:$LD_LIBRARY_PATH
...
#export LD_PRELOAD=/users/fbrunne/miniconda3/envs/rmm_dev/lib/libstdc++.so.6

#    export PYTORCH_CUDA_ALLOC_CONF='use_uvm:True,uvm_oversubscription_ratio:5.0,uvm_access_pattern:gpu_first'

while IFS=, read -r model_name model_type
do
srun bash -c "
    ...
    . ~/projects/deps/patch-venv/bin/activate
    unset PYTHONPATH
    echo 'Python from: $(which python3)'
    python3 -c 'import torch; print(torch.__file__)'
    export PYTORCH_CUDA_ALLOC_CONF='use_uvm:True,uvm_oversubscription_ratio:5.0,uvm_access_pattern:gpu_first'
    python3 library_testing.py \"$model_name\" \"$model_type\"
" < /dev/null
done < models.txt