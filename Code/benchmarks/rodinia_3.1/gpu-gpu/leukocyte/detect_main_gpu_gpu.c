/***********************************************************************************
  GPU-GPU Execution variant for Leukocyte detection and tracking.
  Same as CUDA version but timer starts only after all GPU memory allocation
  and host-to-device transfers (compute_constants) are complete.
  Measures kernel execution + D2H time only.

  This file replaces detect_main.c in the CUDA leukocyte build.
  Compile with the CUDA versions of find_ellipse.c, track_ellipse.c, and
  supporting libraries, same as the original CUDA leukocyte.
 ************************************************************************************/
...
int main(int argc, char ** argv) {
	...
	// ===================== START TIMER (after all setup + H2D transfers) =====================
	struct timeval tv_total_start, tv_total_end;
	cudaDeviceSynchronize();
	gettimeofday(&tv_total_start, NULL);

	// Get GICOV matrices corresponding to image gradients (runs on GPU)
	long long GICOV_start_time = get_time();
	MAT *gicov = GICOV(grad_x, grad_y);
	long long GICOV_end_time = get_time();

	// Dilate the GICOV matrices (runs on GPU)
	...
	double total_time = (tv_total_end.tv_sec - tv_total_start.tv_sec) +
	                    (tv_total_end.tv_usec - tv_total_start.tv_usec) / 1000000.0;

	// Free memory
	free(V); free(ccol); free(crow); free(GICOV_spots);
	free(t); free(G); free(x_result); free(y_result);
	m_free(A); m_free(celly); m_free(cellx);
	m_free(img_dilated); m_free(gicov);
