...
// Global timing variables for data transfer and computation (in ms)
float g_gicov_transfer_time = 0.0f;
float g_gicov_compute_time = 0.0f;
float g_dilate_compute_time = 0.0f;
...
float *GICOV_CUDA(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y) {
	...
	cudaEvent_t transfer_start, transfer_end, compute_start, compute_end;
	...
	// Calculate and accumulate timing
	cudaEventElapsedTime(&g_gicov_transfer_time, transfer_start, transfer_end);
	cudaEventElapsedTime(&g_gicov_compute_time, compute_start, compute_end);
...
float *dilate_CUDA(int max_gicov_m, int max_gicov_n, int strel_m, int strel_n) {
	...
	// Calculate timing
	cudaEventElapsedTime(&g_dilate_compute_time, compute_start, compute_end);
