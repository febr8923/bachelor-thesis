...
// Global timing variables for MGVF data transfer and computation (in ms)
float g_mgvf_transfer_time = 0.0f;
float g_mgvf_compute_time = 0.0f;
...
cudaEvent_t g_mgvf_transfer_start, g_mgvf_transfer_end;
cudaEvent_t g_mgvf_compute_start, g_mgvf_compute_end;
...
void IMGVF_cuda(MAT **I, MAT **IMGVF, double vx, double vy, double e, int max_iterations, double cutoff, int num_cells) {
	...
	cudaEventCreate(&g_mgvf_transfer_start);
	cudaEventCreate(&g_mgvf_transfer_end);
	cudaEventCreate(&g_mgvf_compute_start);
	cudaEventCreate(&g_mgvf_compute_end);

	// Initialize the data on the GPU (includes transfer timing)
	IMGVF_cuda_init(I, num_cells);

	// Start compute timing
	cudaEventRecord(g_mgvf_compute_start, 0);
	...
	// End compute timing
	cudaEventRecord(g_mgvf_compute_end, 0);
	cudaEventSynchronize(g_mgvf_compute_end);
	...
	// Calculate timing
	cudaEventElapsedTime(&g_mgvf_compute_time, g_mgvf_compute_start, g_mgvf_compute_end);

	// Cleanup CUDA events
	cudaEventDestroy(g_mgvf_transfer_start);
	cudaEventDestroy(g_mgvf_transfer_end);
	cudaEventDestroy(g_mgvf_compute_start);
	cudaEventDestroy(g_mgvf_compute_end);
}
...
void IMGVF_cuda_init(MAT **IE, int num_cells) {
	...
	cudaEventRecord(g_mgvf_transfer_start, 0);
	...
	// End transfer timing
	cudaEventRecord(g_mgvf_transfer_end, 0);
	cudaEventSynchronize(g_mgvf_transfer_end);

	// Calculate transfer time
	cudaEventElapsedTime(&g_mgvf_transfer_time, g_mgvf_transfer_start, g_mgvf_transfer_end);
}
