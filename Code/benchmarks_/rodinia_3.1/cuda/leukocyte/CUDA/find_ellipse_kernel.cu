...
__global__ void GICOV_kernel(int grad_m, float *gicov,
	...
	for (k = 0; k < NCIRCLES; k++) {
		...
		for (n = 0; n < NPOINTS; n++) {
			...
			int addr = x * grad_m + y;
			float p = tex1Dfetch<float>(tex_grad_x, addr) * c_cos_angle[n] +
					  tex1Dfetch<float>(tex_grad_y, addr) * c_sin_angle[n];
...
// Helper to create a 1D float texture object from a device buffer
static cudaTextureObject_t create_float_texture(float *device_buf, size_t size_bytes) {
	cudaResourceDesc resDesc;
	memset(&resDesc, 0, sizeof(resDesc));
	resDesc.resType = cudaResourceTypeLinear;
	resDesc.res.linear.devPtr = device_buf;
...
float *GICOV_CUDA(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y) {
	...
	cudaTextureObject_t tex_grad_x = create_float_texture(device_grad_x, grad_mem_size);
	cudaTextureObject_t tex_grad_y = create_float_texture(device_grad_y, grad_mem_size);
	...
	GICOV_kernel <<< num_blocks, threads_per_block >>> (grad_m, device_gicov, tex_grad_x, tex_grad_y);
	...
	// Cleanup memory
	cudaDestroyTextureObject(tex_grad_x);
	cudaDestroyTextureObject(tex_grad_y);
			...
			for(el_j = 0; el_j < strel_n; el_j++) {
				...
				if( (x >= 0) &&
					...
					(c_strel[(el_i * strel_n) + el_j] != 0) ) {
						...
						int addr = (x * img_m) + y;
						float temp = tex1Dfetch<float>(tex_img, addr);
...
float *dilate_CUDA(int max_gicov_m, int max_gicov_n, int strel_m, int strel_n) {
	...
	cudaTextureObject_t tex_img = create_float_texture(device_gicov, max_gicov_mem_size);
	...
	dilate_kernel <<< num_blocks, threads_per_block >>> (max_gicov_m, max_gicov_n, strel_m, strel_n, device_img_dilated, tex_img);
